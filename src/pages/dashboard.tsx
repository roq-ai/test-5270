...
import { getMemberships } from 'apiSdk/memberships';
...
function DashboardPage() {
  ...
  const membershipStat = useSWR<PaginatedInterface<MembershipInterface>>(
    () => `/memberships`,
    () => getMemberships({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Memberships',
      result: membershipStat?.data?.totalCount,
      isLoading: membershipStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Invitations',
